...
class TwoChannelInputResNet(nn.Module):
    def __init__(self, resnet_model):
        super(TwoChannelInputResNet, self).__init__()

        # 1. 通道扩展层：将2通道扩展到3通道
        # self.channel_conv = nn.Conv2d(in_channels=2, out_channels=3, kernel_size=3, padding=1)
...
class Trainer:
    def __init__(self, train_ds, test_ds, args, device):
        ...
        self.args = args
        self.device = device
        self.res_model = resnet34(4)
        self.model = TwoChannelInputResNet(self.res_model).to(device=device)
        self.optimizer = torch.optim.AdamW(self.model.parameters(),lr=self.args.init_lr)
