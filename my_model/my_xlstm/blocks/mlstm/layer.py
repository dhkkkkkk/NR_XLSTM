...
from ...utils import UpProjConfigMixin, SequenceConv1d
...
class mLSTMLayer(nn.Module):
    ...
    def __init__(self, config: mLSTMLayerConfig):
        ...
        self.encout_proj_up = nn.Linear(
            in_features=self.config.embedding_dim,
            out_features=2 * self.config.embedding_dim,
            bias=self.config.bias,
        )
        ...
        self.mlstm_cell = mLSTMCell(        #lstm
    ...
    def forward(self, x: torch.Tensor, enc_output: torch.Tensor = None, **kwargs) -> torch.Tensor:
        B, S, _ = x.shape   #输入为(batch,time,feature)
        ...
        if enc_output is not None:
            enc_mlstm = self.encout_proj_up(enc_output)
            # enc_mlstm, _ = torch.split(enc_inner, split_size_or_sections=self.config._inner_embedding_dim, dim=-1)

            enc_conv = self.conv(enc_mlstm)
            enc_conv_act = self.conv_act_fn(enc_conv)

            q = self.q_proj(x_mlstm_conv_act)
            k = self.k_proj(enc_conv_act)
            v = self.v_proj(enc_mlstm)
    ...
    def step(
        ...
        # mlstm branch
        x_mlstm_conv, conv_state = self.conv.step(x_mlstm, conv_state=conv_state)
        x_mlstm_conv_act = self.conv_act_fn(x_mlstm_conv)
